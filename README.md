...
| 身份 | 应该做什么 | 需要看本仓库源码吗 |
| ---- | ---------- | ------------------ |
| **游戏业务开发者** | 新建项目，`go get` 引入框架，参考第12节示例写业务逻辑 | 不需要 |
| **框架开发者** | 维护/扩展框架本身，修改底层实现 | 需要 |

**游戏业务开发者**只需关注第12节（快速开始），通过 `node.Proxy` / `gate.Proxy` 暴露的 API 编写业务逻辑，框架已处理好连接管理、消息路由、服务注册发现、分布式扩容和平滑重启，无需了解底层实现。

**本仓库源码的学习价值：**

| 主题 | 推荐阅读位置 | 价值 |
| ---- | ------------ | ---- |
| Actor 无锁并发模型 | `cluster/node/actor.go` | ★★★ |
| 分布式消息路由原理 | `internal/link/` + `internal/transporter/` | ★★★ |
| 组件生命周期管理模式 | `container.go` | ★★☆ |
| 可替换接口设计 | `registry/`、`transport/`、`eventbus/` | ★★☆ |

> 建议：先用框架做出游戏，遇到框架满足不了的需求时再回来阅读源码。不要在没做出游戏之前陷入研究架构。
